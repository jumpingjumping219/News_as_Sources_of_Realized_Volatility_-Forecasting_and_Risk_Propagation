# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 17:25:05 2023
...
This module differentiate itself from 2a_Graph_News_LW.py by applying diffent graphs to different features.
- HAR feature 
- News attribute 
"""
...
def preprocess_HAR(news):
    ...
    pp, prefix = add_rolling_horizon_features(pp, news, method='mean', window=21)
...
def preprocess_adj_l(dd_l,ret_df,subdf_dic,tickers, prefix):#################generate the regressors
    ...
    return the aggregated features and original features.
    """
    ...
    for dd in dd_l[5:]:
        ...
        tmp_subdf_2 = []
        # 1 HAR feature
        clms1 = [i for i in subdf.columns if 'lag' in i and 'log' in i]####lag and log should in i, include all the needed feature inside.
        # 2 News attribute
        if this_version.split('_')[2]=='GHAR+NewsPropagation' or this_version.split('_')[2]=='GHAR+NewsPropagationAttribute': ###using the attribute of news to propagate and attribute
            clms2 = [i for i in subdf.columns if (i==f'{prefix}Num_News_shock') or (i==f'{prefix}num_transfer_market') or (i==f'{prefix}Num_scheduled_News') or (i==f'{prefix}Num_unscheduled_News') or (i==f'{prefix}abs_EVENT_SENTIMENT_SCORE') or (i==f'{prefix}SENTIMENT_std') or (i==f'{prefix}media_coverage') or (i==f'{prefix}sentiment_transfer_market_abs') ]
        else:
            clms2 = []            
        print(clms2)
        
        #there are two cases: 1. for the lag volatiity, the adjacency matrix must include the identity matrix
        # 2, for other news attribute, the adjecency matrix not include the identity matrix, so set the identity matrix as 0
        adj_df_l = []  ####################adj_df_1 is for propagate the volatility 
        for para_name in para_name_l:
            if para_name == 'iden':
                ...
                adj_df = pd.read_csv(join('E:/stock_100_glasso_graph/%d.csv' % dd), index_col=0)
            elif para_name == 'news':
        ...
        for k, adj_df in enumerate(adj_df_l):
            # print(adj_df)
            tmp_subdf = pd.DataFrame(np.dot(adj_df, subdf[clms1]), columns=['section'+str(k)+i for i in clms1], index=subdf.index)
            tmp_subdf_l.append(tmp_subdf)

        new_subdf = pd.concat([subdf[[f'{prefix}Target_log', 'dd', 'Ticker',f'{prefix}Num_News_shock',f'{prefix}num_transfer_market',f'{prefix}Num_scheduled_News',f'{prefix}Num_unscheduled_News',f'{prefix}abs_EVENT_SENTIMENT_SCORE',f'{prefix}SENTIMENT_std',f'{prefix}media_coverage',f'{prefix}sentiment_transfer_market_abs']]]+tmp_subdf_l, axis=1)  
            
            
        adj_df_2 = []
        for para_name in para_name_l:
            if para_name == 'iden':
                adj_df = pd.DataFrame(np.zeros((n, n)), index=tickers, columns=tickers) #######################change a lot!!!!it also be zeros and the regressors about the news are added additionally.
            elif para_name == 'global':
                adj_df = pd.DataFrame(np.ones((n, n)), index=tickers, columns=tickers)
                adj_df -= np.identity(n)
                adj_df /= adj_df.sum()
            ...
            adj_df_2.append(adj_df)
            
            
        for k, adj_df in enumerate(adj_df_2):
            # print(adj_df)
            tmp_subdf = pd.DataFrame(np.dot(adj_df, subdf[clms2]), columns=['section'+str(k)+i for i in clms2], index=subdf.index)
            tmp_subdf_2.append(tmp_subdf)            
            
            
        new_subdf = pd.concat([new_subdf]+tmp_subdf_2, axis=1)
       ###########################include more attributes in the dataframe, the regressors will be selected later.
        
        new_subdf_l.append(new_subdf)      #aggregate in the time series.
...
def Train(model_name, ret_df, var_df,df,tickers, subdf_dic, dd, dd_l):####all the information has been included in the subdf_dic
    timestamp = dd_l.index(dd)  ###date_1 is the dd list for all the predicting values
    # split time
    s_p = max(timestamp-1000, 0)  #starting point
    ...
    # df = np.log(df)
    if this_version.split('_')[2]=='GHAR+News' or this_version.split('_')[2]=='GHAR+NewsPropagationAttribute':
        vars_l = [i for i in df.columns if 'section' in i or i=='Num_News_shock' or i=='num_transfer_market' or i=='Num_scheduled_News' or i=='Num_unscheduled_News' or i=='abs_EVENT_SENTIMENT_SCORE' or i=='SENTIMENT_std' or i=='media_coverage' or i=='sentiment_transfer_market_abs']#######the section.....
    else:
        vars_l = [i for i in df.columns if 'section' in i]
    # split data
    ...
    # To avoid negative forecasts of RV, we use historical min values to replace
    ...
    save_path = join('D:\\News_Var_Results', this_version)
...
def connect_pred():
    ...
    sum_path = join('D:\\Var_Results_Sum')
...
if __name__ == '__main__':
    ...
    df=preprocess_adj_l(dd_l,ret_df,subdf_dic,tickers,prefix)
